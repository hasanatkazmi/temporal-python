...
from temporal import (
    ...
    Duration, Instant, TimeZone, Calendar
)
...
def main():
    ...
    try:
        invalid_date = PlainDate(2023, 13, 45)
    except Exception as e:
        print(f"   Invalid date error: {e}")
    
    try:
        invalid_time_str = PlainTime.from_string("25:00:00")
    except Exception as e:
        print(f"   Invalid time string error: {e}")
    
    try:
        invalid_duration_str = Duration.from_string("invalid")
    except Exception as e:
        print(f"   Invalid duration string error: {e}")
    print()
    
    # 9. Advanced usage - business day calculations
